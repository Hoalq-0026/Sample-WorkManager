...
object ImageUtils {
    ...
    fun clearFiles(applicationContext: Context) {
        val outputDirectory = getOutputDirectory(applicationContext)
    ...
    fun createZipFile(applicationContext: Context, files: Array<String>): Uri {
        ...
        val outputDirectionality = getOutputDirectory(applicationContext)
        val outputFile = File(outputDirectionality, name)

        val zipOutputStream = ZipOutputStream(BufferedOutputStream(FileOutputStream(outputFile)))
        compressFile(zipOutputStream, files)
        return Uri.fromFile(outputFile)
    }

    private fun compressFile(zipOutputStream: ZipOutputStream, files: Array<String>) {
    ...
    fun uploadFile(fileUri: Uri) {
        ...
        Log.d(LOG_TAG, "onResponse-Status: ${response?.code()} Body: ${response.body()?.toString()}")
    }
}